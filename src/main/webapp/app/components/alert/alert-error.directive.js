(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('reporttestApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-reporttestApp-error');
                var entityKey = httpResponse.headers('X-reporttestApp-params');
                if (errorHeader) {
